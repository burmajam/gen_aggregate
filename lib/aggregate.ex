...
defmodule Aggregate do
  ...
  def handle_call({:cmd, cmd}, from, %State{buffer: [], transaction: nil}=state) do
    Logger.debug "Executing: #{inspect cmd}"
    lock = make_ref
    GenServer.cast self, {:execute, {cmd, from}}
    {:noreply, %State{state | transaction: lock}}
  end
  def handle_call({:cmd, cmd}, from, %State{}=state) do
  ...
  def handle_call({:commit, transaction, events}, _from, %State{transaction: transaction}=state) do
    Logger.debug "Commiting: #{inspect transaction}"
    state = apply_events events, state
    GenServer.cast self, :process_buffer
    {:reply, :ok, %State{state | transaction: nil, events: []}}
  end
  def handle_call({:commit, t1, _events}, _from, %State{transaction: transaction}=state) do 
  ...
  def handle_cast(:process_buffer, %State{buffer: []}=state), do: {:noreply, state}
  def handle_cast(:process_buffer, %State{buffer: buffer, transaction: nil}=state) do
    lock = make_ref
    {cmd, from} = List.last buffer
    Logger.debug "Processing buffered cmd: #{inspect cmd}"
    buffer = List.delete_at buffer, -1
    GenServer.cast self, {:execute, {cmd, from}}
    {:noreply, %State{state | buffer: buffer, transaction: lock}}
  end
  def handle_cast(:process_buffer, state), do: {:noreply, state}

  def handle_info({:rollback, transaction}, %State{transaction: transaction}=state) do 
